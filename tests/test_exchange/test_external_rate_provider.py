...
class TestSuccessfulRateFetch:
    ...
    def test_get_current_rate_fetches_from_api(self, mock_urlopen: MagicMock) -> None:
    ...
    @patch("src.exchange.external_rate_provider.urlopen")
    def test_get_current_rate_reverse_pair(self, mock_urlopen: MagicMock) -> None:
        mock_urlopen.return_value = _mock_response(
            {
                "base": "EUR",
                "date": "2026-02-25",
...
class TestCaching:
    @patch("src.exchange.external_rate_provider.urlopen")
    def test_cached_rate_avoids_second_api_call(self, mock_urlopen: MagicMock) -> None:
        mock_urlopen.return_value = _mock_response(
            {
                "base": "USD",
                "date": "2026-02-25",
...
class TestFallbackBehavior:
    ...
    @patch("src.exchange.external_rate_provider.urlopen")
    def test_raises_without_fallback_on_api_failure(self, mock_urlopen: MagicMock) -> None:
        mock_urlopen.side_effect = URLError("Network error")
        provider = ExternalRateProvider()
        with pytest.raises(ValueError, match="Failed to fetch"):
            provider.get_current_rate(Currency.USD, Currency.EUR)

    def test_from_fallback_class_method(self, fallback_provider: InMemoryRateProvider) -> None:
...
class TestCOPHandling:
    @patch("src.exchange.external_rate_provider.urlopen")
    def test_cop_uses_static_fallback_rate(self, mock_urlopen: MagicMock) -> None:
    ...
    @patch("src.exchange.external_rate_provider.urlopen")
    def test_cop_reverse_uses_inverse_static_rate(self, mock_urlopen: MagicMock) -> None:
        provider = ExternalRateProvider()
...
class TestGetRateAtDate:
    @patch("src.exchange.external_rate_provider.urlopen")
    def test_historical_rate_fetch(self, mock_urlopen: MagicMock, today: datetime) -> None:
        mock_urlopen.return_value = _mock_response(
            {
                "base": "USD",
                "date": "2025-01-15",
        ...
        provider = ExternalRateProvider()
        result = provider.get_rate_at_date(Currency.USD, Currency.EUR, datetime(2025, 1, 15))
    ...
    @patch("src.exchange.external_rate_provider.urlopen")
    def test_historical_rate_cached(self, mock_urlopen: MagicMock, today: datetime) -> None:
        mock_urlopen.return_value = _mock_response(
            {
                "base": "USD",
                "date": "2025-01-15",
